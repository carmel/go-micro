	...
	if block {
		...
		case err := <-done:
			if err != nil {
				...
				if stopErr != nil {
					logger.Errorf("failed to http client watch stop: %v, error: %+v", target, stopErr)
				}
				return nil, err
			}
		case <-ctx.Done():
			logger.Errorf("http client watch service %v reaching context deadline!", target)
			stopErr := watcher.Stop()
			if stopErr != nil {
				logger.Errorf("failed to http client watch stop: %v, error: %+v", target, stopErr)
			}
	...
	go func() {
		for {
			...
			if err != nil {
				...
				logger.Errorf("http client watch service %v got unexpected error:=%v", target, err)
...
func (r *resolver) update(services []*registry.ServiceInstance) bool {
	...
	for _, ins := range services {
		...
		if err != nil {
			logger.Errorf("Failed to parse (%v) discovery endpoint: %v error %v", r.target, ins.Endpoints, err)
			continue
		}
		if ept == "" {
			continue
	...
	if len(nodes) == 0 {
		logger.Warnf("[http resolver]Zero endpoint found,refused to write,set: %s ins: %v", r.target.Endpoint, nodes)
		return false
	}
