...
type Context interface {
	...
	Midware(midware.Handler) midware.Handler
...
func (c *wrapper) Midware(h midware.Handler) midware.Handler {
	if tr, ok := transport.FromServerContext(c.req.Context()); ok {
		return midware.Chain(c.router.srv.midware.Match(tr.Operation())...)(h)
	}
	return midware.Chain(c.router.srv.midware.Match(c.req.URL.Path)...)(h)
}
