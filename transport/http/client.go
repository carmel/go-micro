...
func (client *Client) invoke(ctx context.Context, req *http.Request, args interface{}, reply interface{}, c callInfo, opts ...CallOption) error {
	...
	if len(client.opts.midware) > 0 {
		h = midware.Chain(client.opts.midware...)(h)
	}
	_, err := h(ctx, args)
	return err
}
...
func DefaultRequestEncoder(_ context.Context, contentType string, in interface{}) ([]byte, error) {
	...
	body, err := cc.GetCodec(name).Marshal(in)
	if err != nil {
		return nil, err
	}
	return body, err
...
// CodecForResponse get cc.Codec via http.Response
func CodecForResponse(r *http.Response) cc.Codec {
	codec := cc.GetCodec(httputil.ContentSubtype(r.Header.Get("Content-Type")))
	if codec != nil {
		return codec
	}
	return cc.GetCodec("json")
}
