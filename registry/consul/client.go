...
func (c *Client) Register(_ context.Context, svc *registry.ServiceInstance, enableHealthCheck bool) error {
	...
	if c.heartbeat {
		go func() {
			...
			if err != nil {
				logger.Errorf("[Consul]update ttl heartbeat to consul failed!err:=%v", err)
			}
				...
				case <-ticker.C:
					...
					if err != nil {
						...
						if err := c.cli.Agent().ServiceRegister(asr); err != nil {
							logger.Errorf("[Consul] re registry service failed!, err=%v", err)
						} else {
							logger.Warnf("[Consul] re registry of service occurred success")
						}
					}
				}
			}
