...
func (d *Discovery) Register(ctx context.Context, service *registry.ServiceInstance) (err error) {
	...
	go func() {
		defer logger.Warnf("Discovery:register_service goroutine quit")
...
func (d *Discovery) register(ctx context.Context, ins *discoveryInstance) (err error) {
	...
	if _, err = d.httpClient.R().
		...
		Post(uri); err != nil {
		d.switchNode()
		logger.Errorf("Discovery: register client.Get(%s)  zone(%s) env(%s) appid(%s) addrs(%v) error(%v)",
	...
	if res.Code != 0 {
		...
		logger.Errorf("Discovery: register client.Get(%v)  env(%s) appid(%s) addrs(%v) code(%v)",
