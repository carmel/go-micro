...
func (d *Discovery) resolveBuild(appID string) *Resolve {
	...
	logger.Debugf("Discovery: AddWatch(%s) already watch(%v)", appID, ok)
	d.once.Do(func() {
		go d.serverProc()
	})
	return r
...
func (d *Discovery) serverProc() {
	defer logger.Debugf("Discovery serverProc quit")
...
func (d *Discovery) renew(ctx context.Context, ins *discoveryInstance) (err error) {
	...
	if _, err = d.httpClient.R().
		...
		Post(uri); err != nil {
		d.switchNode()
		logger.Errorf("Discovery: renew client.Get(%v) env(%s) appid(%s) hostname(%s) error(%v)",
...
func (d *Discovery) cancel(ins *discoveryInstance) (err error) {
	...
	if _, err = d.httpClient.R().
		...
		Post(uri); err != nil {
		d.switchNode()
		logger.Errorf("Discovery cancel client.Get(%v) env(%s) appid(%s) hostname(%s) error(%v)",
	...
	if res.Code != _codeOK {
		...
		logger.Warnf("Discovery cancel client.Get(%v)  env(%s) appid(%s) hostname(%s) code(%v)",
...
func (d *Discovery) polls(ctx context.Context) (apps map[string]*disInstancesInfo, err error) {
	...
	if _, err = d.httpClient.R().
		...
		d.switchNode()
		logger.Errorf("Discovery: client.Get(%s) error(%+v)", reqURI, err)
		return nil, err
	}

	if res.Code != _codeOK {
		if res.Code != _codeNotModified {
			logger.Errorf("Discovery: client.Get(%s) get error code(%d)", reqURI, res.Code)
		}
	...
	for _, app := range res.Data {
		if app.LastTs == 0 {
			err = ErrServerError
			logger.Errorf("Discovery: client.Get(%s) latest_timestamp is 0, instances:(%+v)", reqURI, res.Data)
			return
		}
	}

	logger.Debugf("Discovery: successfully polls(%s) instances (%+v)", reqURI, res.Data)
	apps = res.Data
	return
}
