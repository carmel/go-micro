...
func TestTraceError(t *testing.T) {
	...
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "go-micro/errs_test.grandson", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "go-micro/errs_test.child", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "go-micro/errs_test.parent", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)
}
...
func TestTraceErrorSetStackSkip(t *testing.T) {
	...
	line, isPrefix, err = br.ReadLine()
	t.Log(string(line))
	assert.Contains(t, string(line), "go-micro/errs_test.TestTraceErrorSetStackSkip")
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "go-micro/errs_test.TestTraceErrorSetStackSkip.func1", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "go-micro/errs_test.TestTraceErrorSetStackSkip", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)
}
