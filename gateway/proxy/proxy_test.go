...
import (
	...
	"go-micro/gateway/midware/logging"
)
...
func TestProxy(t *testing.T) {
	...
	middlewareFactory := func(c *config.Midware) (midware.MidwareV2, error) {
		return logging.Midware(c)
	}
...
func TestRetryBreaker(t *testing.T) {
	...
	clientFactory := func(*config.Endpoint) (client.Client, error) {
		dummyClient := RoundTripperCloserFunc(func(req *http.Request) (resp *http.Response, _ error) {
			opt, _ := midware.FromRequestContext(req.Context())
			defer func() {
