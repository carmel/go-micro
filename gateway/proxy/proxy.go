...
import (
	...
	"github.com/carmel/microservices/gateway/router/mux"
	...
	"github.com/carmel/microservices/transport/http/status"
...
func writeError(w http.ResponseWriter, r *http.Request, err error, labels midware.MetricsLabels) {
	var statusCode int
	switch {
	...
	case errors.Is(err, context.DeadlineExceeded):
		statusCode = 504
	default:
		logger.Errorf("Failed to handle request: %s: %+v", r.URL.String(), err)
		statusCode = 502
	}
...
func notFoundHandler(w http.ResponseWriter, r *http.Request) {
	...
	logger.With(
		...
		"code", code,
	).Log(r.Context(), slog.LevelError, message)
...
type Proxy struct {
	...
	middlewareFactory midware.FactoryV2
}

// New is new a gateway proxy.
func New(clientFactory client.Factory, middlewareFactory midware.FactoryV2) (*Proxy, error) {
	p := &Proxy{
...
func (p *Proxy) buildMiddleware(ms []*config.Midware, next http.RoundTripper) (http.RoundTripper, error) {
	for i := len(ms) - 1; i >= 0; i-- {
		m, err := p.middlewareFactory(ms[i])
		if err != nil {
			if errors.Is(err, midware.ErrNotFound) {
				logger.Errorf("Skip does not exist midware: %s", ms[i].Name)
				continue
			}
			return nil, err
		}
...
func splitRetryMetricsHandler(e *config.Endpoint) (func(int), func(int, error)) {
	labels := midware.NewMetricsLabels(e)
	success := func(i int) {
		if i <= 0 {
			return
		}
...
func (p *Proxy) buildEndpoint(e *config.Endpoint, ms []*config.Midware) (_ http.Handler, _ io.Closer, retError error) {
	...
	return http.Handler(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		reqOpts := midware.NewRequestOptions(e)
		ctx := midware.NewRequestContext(req.Context(), reqOpts)
...
func receivedBytesAdd(labels midware.MetricsLabels, received int64) {
...
func sentBytesAdd(labels midware.MetricsLabels, sent int64) {
...
func requestsTotalIncr(labels midware.MetricsLabels, statusCode int) {
...
func requestsDurationObserve(labels midware.MetricsLabels, seconds float64) {
...
func retryStateIncr(labels midware.MetricsLabels, success bool) {
	if success {
...
func (p *Proxy) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	defer func() {
		if err := recover(); err != nil {
			...
			logger.Errorf("panic recovered: %s", buf[:n])
