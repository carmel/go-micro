...
func (f *FileLoader) Load(_ context.Context) (*configv1.Gateway, error) {
	logger.Infof("loading config file: %s", f.confPath)
...
func (f *FileLoader) Watch(fn OnChange) {
	logger.Infof("add config file change event handler")
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *FileLoader) executeLoader() error {
	logger.Log(logger.INFO, "execute config loader")
	f.lock.RLock()
...
func (f *FileLoader) watchproc(ctx context.Context) {
	logger.Log(logger.INFO, "start watch config file")
	for {
		select {
		case <-ctx.Done():
			return
		...
		func() {
			...
			if sha256hex != f.confSHA256 {
				logger.Infof("config file changed, reload config, last sha256: %s, new sha256: %s", f.confSHA256, sha256hex)
				if err := f.executeLoader(); err != nil {
					logger.Errorf("execute config loader error with new sha256: %s: %+v, config digest will not be changed until all loaders are succeeded", sha256hex, err)
					return
				}
				f.confSHA256 = sha256hex
				return
