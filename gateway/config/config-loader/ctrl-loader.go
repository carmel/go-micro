...
func prepareCtrlService(in string) []string {
	...
	for _, part := range parts {
		...
		if err != nil {
			logger.Warnf("Failed to parse control service url %s: %s, will skip this one", part, err)
			continue
		}
		out = append(out, u.String())
	}
	if len(out) == 0 {
		logger.Warnf("No control service url found, control service will not be available")
	}
...
func (c *CtrlConfigLoader) Load(ctx context.Context) (err error) {
	...
	if err != nil {
		if err == errNotModified {
			logger.Infof("Skip loading config, %q-%q config is up to date: %q", c.advertiseName, c.advertiseAddr, c.lastVersion.String())
			return nil
		}
		return err
	}
...
func (c *CtrlConfigLoader) getAdvertiseAddr() string {
	...
	if err != nil {
		logger.Errorf("%q There was a problem with the IP %+v", c.advertiseName, err)
		return ""
	}
	logger.Infof("%s uses IP %s\n", c.advertiseName, advAddr)
	return advAddr
}
...
func (c *CtrlConfigLoader) load(ctx context.Context) ([]byte, error) {
	...
	logger.Infof("%s is requesting config from %s with params: %+v", c.advertiseName, c.ctrlService, params)
...
func (c *CtrlConfigLoader) loadFeatures(ctx context.Context) ([]byte, error) {
	...
	params.Set("gateway", c.advertiseName)
	params.Set("ip_addr", c.advertiseAddr)
	logger.Infof("%s is requesting features from %s with params: %+v", c.advertiseName, c.ctrlService, params)
...
func (c *CtrlConfigLoader) Run(ctx context.Context) {
	...
	for {
		if err := c.Load(ctx); err != nil {
			logger.Warnf("Failed to load config, %q-%q, %+v", c.advertiseName, c.advertiseAddr, err)
			continue
		}
		if err := c.LoadFeatures(ctx); err != nil {
			logger.Warnf("Failed to load gateway features, %q-%q, %+v", c.advertiseName, c.advertiseAddr, err)
		}
		select {
		case <-ctx.Done():
			return
