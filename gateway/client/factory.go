...
func (na *nodeApplier) apply(ctx context.Context) error {
	...
	for _, backend := range na.endpoint.Backends {
		...
		case "discovery":
			...
			if existed {
				logger.Infof("watch target %+v already existed", target)
			}
		default:
...
func (na *nodeApplier) Callback(services []*registry.ServiceInstance) error {
	...
	for _, ser := range services {
		...
		if err != nil || addr == "" {
			logger.Errorf("failed to parse endpoint: %v/%s: %v", ser.Endpoints, scheme, err)
			continue
		}
...
func (na *nodeApplier) Cancel() {
	logger.Infof("Closing node applier for endpoint: %+v", na.endpoint)
